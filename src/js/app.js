        ...
        socket.on('connect', () => {
            socket.on('chat message', msg => {
                ...
                if(msg.user === this.state.username){
                    newMsgs.push(<div data-aos='slide-left' data-aos-duration='220' data-aos-once='true' data-aos-offset='-10' key={this.state.messages.length+1} className='user message'>{msg.message}</div>);
                }else{
                    newMsgs.push(<div data-aos='slide-right' data-aos-duration='220' data-aos-once='true' data-aos-offset='-10' key={this.state.messages.length+1} className='other message'>{`${msg.user} : ${msg.message}`}</div>);
                }
